(() => {
  ...
  function Widget() {
    ...
    return /* @__PURE__ */ figma.widget.h(Frame, {
      ...
      return Category(category.name, category.color, Status, `${category.name}`);
    }))));
  }
  function Category(name, color, status, category_key) {
    const hideCategory = status == "Current" && name == "Writing" || status == "Future" && name == "Craft";
  ...
  function Skill(name, category, color, skill_description, skill_key, status, showLevels, voteMap) {
    ...
    const hideSkill = status == "Current" && category == "Writing" || status == "Future" && category == "Craft";
    return /* @__PURE__ */ figma.widget.h(Frame, {
      name: `Skill-${name}`,
      key: `Skill-${skill_key}`,
      width: 250,
